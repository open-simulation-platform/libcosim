...
size_t slave_value_provider::get_real_samples(variable_index variableIndex, step_number fromStep, gsl::span<double> values, gsl::span<step_number> steps)
{
    std::lock_guard<std::mutex> lock(lock_);
    return get_samples<double>(variableIndex, realIndexes_, realSamples_, fromStep, values, steps);
}

size_t slave_value_provider::get_int_samples(variable_index variableIndex, step_number fromStep, gsl::span<int> values, gsl::span<step_number> steps)
{
    std::lock_guard<std::mutex> lock(lock_);
...
template<typename T>
size_t slave_value_provider::get_samples(
