 ...
 *
 *  This is an asynchronous variant of `cosim::slave`, typically used for
 *  remote control of slaves (e.g. through some form of RPC).
 *
 *  The interface is completely analogous to `cosim::slave`, and the functions
 *  in the two classes are for the most part subject to the same constraints
 *  and requirements.  We therefore refer the reader to the `cosim::slave`
...
public:
     ...
     *  in `cosim::slave`, to allow for more efficient transfer of variable
